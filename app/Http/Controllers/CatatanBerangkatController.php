...
class CatatanBerangkatController extends Controller
{
    public function index(Request $request, LayananBonusMingguan $bonus): Response|RedirectResponse
    {
        ...
        return Inertia::render('Catatan', [
            ...
            'target'  => substr($request->user()->pengaturan?->on_time_target ?: '06:30:00', 0, 5),
            'balance' => (int) $anak->transaksiPoin()->sum('amount'),
            'today'   => CarbonImmutable::now($timezone)->toDateString(),
        ]);
    }

    public function store(Request $request, LayananPoin $layananPoin, LayananBonusMingguan $bonus): RedirectResponse
    {
        ...
        $hariSekolah     = $user->pengaturan?->school_days ?: [1, 2, 3, 4, 5];
        ...
        $exists = $anak->catatanBerangkat()->whereDate('tanggal_berangkat', $data['tanggal_berangkat'])->exists();

        if ($exists) {
            return back()->withErrors(['tanggal_berangkat' => 'Tanggal itu sudah memiliki catatan. Gunakan Edit untuk memperbaiki jamnya.']);
        }

        $createdAt = CarbonImmutable::createFromFormat('Y-m-d H:i', $data['tanggal_berangkat'] . ' ' . $data['jam_berangkat'], $timezone);

        DB::transaction(function () use ($data, $user, $anak, $layananPoin, $createdAt): void {
            $jam            = $data['jam_berangkat'] . ':00';
            $target         = $user->pengaturan?->on_time_target ?: '06:30:00';
            $aturanSnapshot = $layananPoin->snapshot($layananPoin->ambilAturanAktif($user->id));
    ...
    public function update(Request $request, CatatanBerangkat $catatanBerangkat, LayananPoin $layananPoin, LayananBonusMingguan $bonus): RedirectResponse
    {
        ...
        DB::transaction(function () use ($data, $request, $catatanBerangkat, $layananPoin): void {
            ...
            $record->update(['jam_berangkat' => $jam, 'note' => $data['note'] ?? null, 'aturan_poin_snapshot' => $aturanSnapshot, 'poin_didapat' => $newPoints]);
