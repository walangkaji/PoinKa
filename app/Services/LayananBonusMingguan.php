...
class LayananBonusMingguan
{
    ...
    public function proses(Anak $anak, ?CarbonImmutable $referensi = null): void
    {
        ...
        for ($awalMinggu = CarbonImmutable::parse($tanggalPertama, $timezone)->startOfWeek(); $awalMinggu->lessThanOrEqualTo($akhirMingguTerakhir->startOfWeek()); $awalMinggu = $awalMinggu->addWeek()) {
            $this->prosesMinggu($anak, $user, $awalMinggu);
        }
    }

    private function prosesMinggu(Anak $anak, $user, CarbonImmutable $awalMinggu): void
    {
        ...
        DB::transaction(function () use ($anak, $weekKey, $akhirMinggu, $konfigurasiBonus, $bonusDiinginkan): void {
